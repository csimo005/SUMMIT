...
def train(cfg, output_dir='', run_name=''):
    ...
    train_source = cfg.TRAIN.XMUDA.lambda_seg > 0 or cfg.TRAIN.XMUDA.lambda_xm_src
    train_target = cfg.TRAIN.XMUDA.lambda_xm_trg > 0 or cfg.TRAIN.XMUDA.lambda_pl > 0 or cfg.TRAIN.XMUDA.lambda_minent > 0 or cfg.TRAIN.XMUDA.lambda_ent > 0 or cfg.TRAIN.XMUDA.lambda_div > 0
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        if train_source:
            ...
            seg_loss_src_2d = cfg.TRAIN.XMUDA.lambda_seg * F.cross_entropy(preds_2d['seg_logit'], data_batch_src['seg_label'], weight=class_weights)
            seg_loss_src_3d = cfg.TRAIN.XMUDA.lambda_seg * F.cross_entropy(preds_3d['seg_logit'], data_batch_src['seg_label'], weight=class_weights)
