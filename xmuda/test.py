...
def parse_args():
    ...
    parser.add_argument('--draw', action='store_true', help='draw  image of segmenation')
    parser.add_argument('--video', action='store_true', help='Create video of segmentation')
    parser.add_argument(
        'opts',
...
def test(cfg, args, output_dir=''):
    ...
    target_dataloader = build_dataloader(cfg, mode='test', domain='target')
    ...
    img_path = None
    if args.draw:
        img_path = osp.join(output_dir, 'imgs_new')
        os.makedirs(img_path, exist_ok=True)
    
    video_path = None
    if args.video:
        video_path = osp.join(output_dir, 'videos')
    ...
    if args.source:
        ...
        validate(cfg, model_2d, model_3d, source_dataloader, test_metric_logger)
    logger.info('Target Test Set')
    validate(cfg, model_2d, model_3d, target_dataloader, test_metric_logger, pselab_path=pselab_path, img_path=img_path, video_path=video_path)
