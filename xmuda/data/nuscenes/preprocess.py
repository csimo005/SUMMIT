...
def preprocess(nusc, split_names, root_dir, out_dir,
    ...
    for i, sample in enumerate(nusc.sample):
        ...
        alpha = np.random.random_sample()*0.18 - 0.09
        rotX = np.asarray([[1, 0, 0],
                           [0, np.cos(alpha), -np.sin(alpha)],
                           [0, np.sin(alpha),  np.cos(alpha)]])
        beta = np.random.random_sample()*0.18 - 0.09 
        rotY = np.asarray([[np.cos(beta), 0, np.sin(beta)],
                           [0, 1, 0],
                           [-np.sin(beta), 0, np.cos(beta)]])
        gamma = np.random.random_sample()*0.18 - 0.09
        rotZ = np.asarray([[np.cos(gamma), -np.sin(gamma), 0],
                           [np.sin(gamma), np.cos(gamma), 0],
                           [0, 0, 1]])
        calib_infos['cam2ego_rotation'] = (Quaternion(calib_infos['cam2ego_rotation'])*Quaternion(matrix=rotX)*Quaternion(matrix=rotY)*Quaternion(matrix=rotZ)).elements
...
if __name__ == '__main__':
    ...
    out_dir = '/data/AmitRoyChowdhury/xmuda/nuscenes_noisy_5'
